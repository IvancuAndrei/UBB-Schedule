...
app.get("/api/:specializare/:an/:grupa/:semigrupa", async (req, res) => {
  ...
  let semester;
  let year;
  if (currDate.getMonth() >= 2 && currDate.getMonth() <= 9) {
    semester = `${currDate.getFullYear() - 1}-2`;
  } else if (currDate.getMonth() == 1) {
    semester = `${currDate.getFullYear() - 1}-1`;
  } else
  {
  ...
  console.log(sentURL);
  const tab = await getGroupSchedule(sentURL, req.params.grupa, req.params.semigrupa);
  res.json(tab);
});
...
app.get("/api/groups/:specializare/:an/:grupa/:semigrupa", async (req, res) => {
  ...
  const sentURL = `https://www.cs.ubbcluj.ro/files/orar/${semester}/tabelar/${req.params.specializare}${req.params.an}.html`;
  const groups = await getGroups(sentURL);
  res.json(groups);
});
